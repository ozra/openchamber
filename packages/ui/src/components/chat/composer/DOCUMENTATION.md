...
changed, desktop Enter sends, while mobile and the expanded desktop composer
require Ctrl/Cmd+Enter. An explicit choice applies across the shared composers:
Enter sends when enabled and inserts a newline when disabled. Ctrl/Cmd+Enter
sends in either configured mode. Shift+Enter always inserts a newline.
