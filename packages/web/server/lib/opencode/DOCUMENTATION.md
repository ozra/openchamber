...
- Shared sidebar preferences are stored as validated top-level fields: `sidebarProjectDisplayMode`, `sidebarSessionGroupingMode`, `sidebarProjectSortOrder`, `sidebarShowRecentSection`, `sidebarKeepOpen`, `sidebarHideHeaderNewSession`, and `sidebarActionsAlwaysVisible`. Device-local picker selection and sticky-header state do not enter either settings file.
